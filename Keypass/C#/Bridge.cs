...
namespace KeyPass
{
    public class Bridge
    {
        ...
        public static void EnviaInfoEmpresa(int Terminal, TextBox edLog)
        {
            ...
            {
                if (rede.KeyPass_EnviaEmpresa(EmpregadorDados.PessoaTipo, EmpregadorDados.Pessoa, EmpregadorDados.Nome, EmpregadorDados.Cei, EmpregadorDados.Endereco))
                {
                    rede.KeyPass_InsereFuncionarioMasterListaEnvio(TerminalDados.SupervisorPis, TerminalDados.SupervisorCodigo.ToString(), TerminalDados.SupervisorSenha.ToString());
                    rede.KeyPass_EnviaFuncionariosMasterListaEnvio();
                }
            }
        }
        ...
        public static bool EnviarFuncionariosTerminal(int Terminal, Principal form, TextBox edLog)
        {
            ...
            Rede rede = new Rede(TerminalDados.IP, TerminalDados.Porta, edLog);
            //if (!rede.KeyPass_Conectar()) return Result;
            ...
            if (Result)
            {
                if (rede.KeyPass_EnviaFuncionariosListaEnvio())
                {
                    db.Atualizar_TerminaisFuncionarios(Terminal, FuncionariosAtualizados);
                }
            }

            //rede.KeyPass_Desconectar();
            return Result;
        }
        ...
        static bool EnviarFuncionario(DBApp db, Rede rede, int Terminal, int Funcionario)
        {
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                ...
                if (Pis != string.Empty)
                {
                    rede.KeyPass_InsereFuncionarioListaEnvio(dr["FUNC_NOME"].ToString().Trim(), Pis, Cracha);
        ...
        public static bool ExcluirFuncionariosSelecao(int Terminal, ListView listFuncionarios, TextBox edLog)
        {
            ...
            for (int i = 0; i <= listFuncionarios.SelectedIndices.Count - 1; i++)
            {
                ...
                if (rede.KeyPass_ExcluirFuncionario(Nome, Pis))
                {
        ...
        public static void ImportarMarcacoesTerminal(int Terminal, TextBox edLog)
        {
            ...
            if (rede.KeyPass_ImportarMarcacoes(Terminal, TerminalDados.Descricao, marcacoes))
            {
        ...
        public static void EnviaConfiguracao(int Terminal, TextBox edLog)
        {
            ...
            Rede rede = new Rede(TerminalDados.IP, TerminalDados.Porta, edLog);
            rede.KeyPass_EnviaConfiguracao();
        }
