...
public class EnviaQtdPapel extends Comando {
	
	public FlagErroComando execute(String ip, String porta, String cpf, String hash, int qtdPapel) {
		...
		byte[] buffer = Comando.criarPacoteCabecalho(CodigosComandos.START_PC, CodigosComandos.ENVIAR_QTD_PAPEL, 
													 Conversor.intToByteArray(qtdPapel, 4), new byte[4], Conversor.cpfToByte(cpf), 
		...
		try {
			...
			try {
				...
				flagErroComando = super.tratarResposta(CodigosComandos.ENVIAR_QTD_PAPEL, respostaREP, qtdBytesRecebidos, Protocolo.QTD_BYTES_CABECALHO_CRIPTOGRAFADO);
			} catch (IOException e) {
				e.printStackTrace();
			}
			tcp.finalizaConexao();
