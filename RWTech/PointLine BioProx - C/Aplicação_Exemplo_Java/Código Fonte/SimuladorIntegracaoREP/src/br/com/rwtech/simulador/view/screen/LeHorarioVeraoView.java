...
import br.com.rwtech.simulador.protocolorep.comandos.LeHorarioVerao;
import br.com.rwtech.simulador.utils.Validacao;

public class LeHorarioVeraoView extends DefaultView {

	private static final long serialVersionUID = 1L;

	public LeHorarioVeraoView(String title) {
		super(title);
	}
		...
		this.enviar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (Validacao.isDadosConexaoOK(ip.getText(), porta.getText())) {
					...
					LeHorarioVerao leHorarioVerao = new LeHorarioVerao(ip.getText(), porta.getText(), 
																	   EquipamentoManager.getInstance().getEquipamento().getCpf(), 
																	   EquipamentoManager.getInstance().getEquipamento().getChaveCriptografica());
					FlagErroComando resultado = leHorarioVerao.execute();
