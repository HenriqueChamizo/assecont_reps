...
namespace RepProtocolTestSuite
{
    ...
    class RepProtocol
    {
        ...
        public void RemoveEmployee(string PIS, out string resp)
        {
            Dictionary<string, string> postData = new Dictionary<string, string>();
            postData.Add("PIS", PIS);
            resp = server.DoPostRequest(baseAddress + "MT/Employee/RemoveEmployee", postData, this, new ErrorCodes[] { ErrorCodes.MT_RES_ERROR_NOT_ENROLLED_EMPLOYEE });
        }

        public void SetEmployee(Empregado empregado, out string resp)
        {
            Dictionary<string, string> postData = new Dictionary<string, string>();
            ...
            resp = server.DoPostRequest(baseAddress + "MT/Employee/SetEmployee", postData, this, new ErrorCodes[] { ErrorCodes.MT_RES_ERROR_ENROLLED_ID, ErrorCodes.MT_RES_ERROR_ENROLLED_KEYBOARD, ErrorCodes.MT_RES_ERROR_ENROLLED_PIS });
        }

        public void EditEmployee(Empregado empregado, out string resp)
        {
            Dictionary<string, string> postData = new Dictionary<string, string>();
            postData.Add("PIS", empregado.PIS);
            postData.Add("ID", empregado.ID);
            ...
            postData.Add("CNTLS", empregado.CNTLS);
            postData.Add("KBD", empregado.KBD);
            resp = server.DoPostRequest(baseAddress + "MT/Employee/EditEmployee", postData, this);
        }

        public void SetTime(DateTime time, out string resp)
        {
            Dictionary<string, string> postData = new Dictionary<string, string>();
            ...
            resp = server.DoPostRequest(baseAddress + "MRP/RTC/SetTime", postData, this);
            //resp = server.DoPostRequest(baseAddress + "MRP/SetTime", postData, this);
        }
        ...
        public void SetEmployer(Empregador empregador)
        {
            Dictionary<string, string> postData = new Dictionary<string, string>();
            ...
            string resp = server.DoPostRequest(baseAddress + "MT/Employer/SetEmployer", postData, this);
        }

        public void EditEmployer(Empregador empregador, out string resp)
        {
            Dictionary<string, string> postData = new Dictionary<string, string>();
            postData.Add("CNPJ", empregador.CNPJ);
            postData.Add("CEI", empregador.CEI);
            ...
            postData.Add("NOME", empregador.Nome);
            postData.Add("END", empregador.Endereco);
            resp = server.DoPostRequest(baseAddress + "MT/Employer/EditEmployer", postData, this);
        }

        public void SetPassword(string password)
        {
            password = BitConverter.ToString(server.EncryptByMasterKey(password)).Replace("-", "");
            Dictionary<string, string> postData = new Dictionary<string, string>();
            postData.Add("PSSWD", password);
           server.DoPostRequest(baseAddress + "USER/SetPasswd", postData, this);
        }

        public byte[] GetEncryptedFirmware(int rangeStart, int rangeEnd)
        {
            return Utils.HexStringToByteArray(new RestServices(10000).DoGetRequest(baseAddress + "MRP/FW/FROM/" + rangeStart.ToString("X8") + "/TO/" + rangeEnd.ToString("X8")));
            
        }

        public byte[] GetEncryptedFirmware(Action<int, int> progressCallback)
        {
            int fwSize = 1024 * 1024;
            int blockSize = 2048;
            int maxFailures = 5;
            int failures = 0;
            cancelOperation = false;
            MemoryStream ms = new MemoryStream(fwSize);
            using (BinaryWriter bw = new BinaryWriter(ms))
            {
                for (int i = 0; i < fwSize; i += blockSize)
                {
                    if (progressCallback != null)
                    {
                        progressCallback(i, fwSize);
                    }
                    if (cancelOperation)
                    {
                    ...
                    try
                    {
                        bw.Write(GetEncryptedFirmware(i, i + blockSize - 1));
                ...
                if (progressCallback != null)
                {
                    progressCallback(fwSize, fwSize);
                }
            }
            return ms.ToArray();
        }
        ...
        public void UpdateFirmware(byte[] firmware, Action<int, int> progressCallback)
        {
            server.Timeout = 10 * 60 * 1000;
            server.SendFile(baseAddress + "PostUpdateVersion", firmware, progressCallback);
            server.Timeout = RestServices.DefaultTimeout;
        }

        public void SendSignature(byte[] signature)
        {
            server.Timeout = 10 * 1000;
            Dictionary<string, string> postData = new Dictionary<string, string>();
            postData.Add("SIGN", BitConverter.ToString(signature).Replace("-", ""));
            string resp = server.DoPostRequest(baseAddress + "FWUPDATE", postData, this);
            server.Timeout = RestServices.DefaultTimeout;
        }
