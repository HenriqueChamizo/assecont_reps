...
uses
  ...
  Dialogs, StdCtrls, Buttons, ExtCtrls, ScktComp, ComCtrls, Math;
...
var
  ...
  _gTipoEnvio : String; // Guarda o tipo de envio, é carregado quando o usuário.
  ...
  for i := lstAdicionar.Items.Count -1 downto 0 do
    ...
    begin
      ...
      begin
        ...
        mmInfo.Lines.Add('Exemplo: 01+EH+00+09/07/12 16:44:00]00/00/00]00/00/00' +
          ' Envia a data de 09/07/12 às 16:44:00 sem horário de verão.');
        mmInfo.Lines.Add('');
...
begin
  ...
  begin
    if(pTipoPacote = 'Configurações') then
      ...
      Result := '01+EH+00+09/07/12 16:44:00]00/00/00]00/00/00'
    else if(pTipoPacote = 'Biometria') then
      Result := '01+ED+00+'
    else if(pTipoPacote = 'Cartão') then
...
begin
  try
    frmClienteGPRS.mmEnviadosString.Lines.Add(frmClienteGPRS.edtConfirmarEnvio.Text);
    frmClienteGPRS.mmEnviadosString.Lines.Add('');
    _rPacoteBytes := stringToBytes(frmClienteGPRS.edtConfirmarEnvio.Text);
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              btnEnviarPacoteClick(Self);proEnviarPacote;
              Delay(200);
