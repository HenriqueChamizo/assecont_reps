    ...
    public class Rede
    {
        ...
        private bool Connection_Verification()
        {
            ...
            while (bEndVerification == false)
            {
                ...
                {
                    case enuConnectionState.None:
                        {
                        }
                        break;
                    case enuConnectionState.ConnectionAttempt:
                        {
                        }
                        break;
                    case enuConnectionState.ConnectionAttemptFailed:
                        {
                        ...
                        break;
                    case enuConnectionState.Connected:
                        {
                    ...
                    case enuConnectionState.Disconnected:
                        {
                            this.AddLog("DESCONECTADO COM O TERMINAL");
                            bEndVerification = true;
                            bReturn = true;
                        }
                        break;
                    case enuConnectionState.DataSent:
                        {
                            bReturn = true;
                        }
                        break;
                    case enuConnectionState.ReceivedData:
                        {
                            bEndVerification = true;
                            bReturn = true;
                        }
        ...
        public void IdData_EnviaEmpresa(byte IdentificadorTipo, string Identificador, string Nome, ulong Cei, string Endereco)
        {
            try
            {
                ...
                int iReturn = this.objIDSysR30.PacketAvail(this.objSocketClient.ReceivedData);
            ...
            // o parâmetro Form é para processar se o usuário pressionou <ESC>
        {
            ...
            try
            {
                ...
                iReturn = this.objIDSysR30.PacketAvail(this.objSocketClient.ReceivedBytes);
                                ...
                                case 3:
                                    {
                                        ...
                                        if ((strPIS != "") && (strPIS[0] >= Convert.ToChar("0")) && (strPIS[0] <= Convert.ToChar("9")))
                                        {
                                            ...
                                            AddLogUnformatted(String.Format("MARCAÇÃO RECEBIDA PIS: {0} {1} {2}", uiIdx, strPIS, strDataHora));
                                            marcacoes.Add(String.Format("{0} {1}", strPIS, strDataHora));
                                            marcacoesnovas.Add(String.Format("{0} {1}", strPIS, strDataHora));
                                        }
                                        else
                ...
                if (uiTotalLogType3 > 0)
                {
                    ...
                    ProcessarMarcacoes(db, Grupo, marcacoesnovas);
                }
        ...
        public int IdData_EnviarFuncionarioSemBiometria(string strPIS, string strUserName, uint uiKeyCode, string strBarCode, byte byFacilityCode, uint uiProxCode, byte byUserType, string strPassword, bool SobreporSeExistir)
        {
            ...
            try
            {
                ...
                iReturn = this.objIDSysR30.PacketAvail(this.objSocketClient.ReceivedData);
