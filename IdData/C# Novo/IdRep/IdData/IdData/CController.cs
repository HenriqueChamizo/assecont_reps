...
namespace IdData
{
    public class CController
    {
        ...
        public ECommand _eCurrentCommand;
        public EConnectionState _eCurrentConnectionState;
        public CIDSysR30 _objIDSysR30;
        public CSocketClient _objSocketClient;
        public List<string> _lstrEventData;

        public string _strCommand_Error_Message;
        ...
        public void FormatString(string p_strValue, string p_strMask, ref string p_strValueFormated)
        {
        ...
        public bool SendBuffer(byte[] rgbyTXBuffer)
        {
            try
            {
                bool bReturn = false;
        ...
        public void SocketClient_OnConnect()
        {
            this._eCurrentConnectionState = EConnectionState.Connected;
        }

        public void SocketClient_OnDisconnect()
        {
            this._eCurrentConnectionState = EConnectionState.Disconnected;
        }

        public void SocketClient_OnRead(byte[] rgbyTXBuffer, ref int iReturn)
        {
            try
            {
                int iCommand_Error_Code = this._objIDSysR30.PacketAvail(rgbyTXBuffer);
                iReturn = iCommand_Error_Code;
        ...
        public void SocketClient_OnError(string strErroMessage, int iErroCode)
        {
        ...
        public bool AddUser(string strPIS, string strUserName, uint uiKeyCode, string strBarCode, byte byFacilityCode, ushort usProxCode, byte byUserType, byte byAccessType, string strPassword, System.IO.MemoryStream msPhoto, ushort usSizeSample, byte byQuantitySamples, byte[] rgbyBiometrics, ref byte[] bytesReturn)
        {
            try
            {
                byte[] rgbyBuffer = this._objIDSysR30.AddUser(strPIS, strUserName, uiKeyCode, strBarCode, byFacilityCode, usProxCode, byUserType, byAccessType, strPassword, msPhoto, usSizeSample, byQuantitySamples, rgbyBiometrics);

                if (this.SendBuffer(rgbyBuffer) == false)
                {
                    return false;
                }
                bytesReturn = rgbyBuffer;
                ...
                int ireturn = 0;
                SocketClient_OnRead(rgbyBuffer, ref ireturn);
                if (ireturn == 0)
                    return true;
                else
                    return false;
        ...
        public bool ChangeUserData(string strPIS, string strNewPIS, string strUserName, uint uiKeyCode, string strBarCode, byte byFacilityCode, ushort usProxCode, byte byUserType, string strPassword, System.IO.MemoryStream msPhoto, ushort usSizeSample, byte byQuantitySamples, byte[] rgbyBiometrics, ref byte[] bytesReturn)
        {
            try
            {
        ...
        public bool SetEmployer(byte byIdentifyType, string strCNPJ_CPF, ulong strCEI, string strEmployerName, string strEmployerAddress)
        {
            try
            {
        ...
        public bool GetEmployerData()
        {
            try
            {
                ...
                {
                    strMask = "##.###.###/####-##";
                    this.FormatString(strCNPJ_CPF.ToString(), strMask, ref strFormatedValue);
                    this._lstrEventData.Add("CNPJ : " + strFormatedValue);
                }
                else
                {
