...
namespace Digicon
{
    public class Bridge : AssepontoRep.Bridge
    {
        ...
        {
            consoleLog = edLog;
            IPLocal = GetLocalIPAddress(); // Wr.Classes.Net.getLocalIPAddress();
        ...
        {
            log.AddLog(strMensagem + "\r\n");
            consoleLog.ScrollToCaret();
        }

        public void Form1_StatusChanged(WriteLogArgs args)
        {
            consoleLog.BeginInvoke(new AtualizaStatusCallback(this.AtualizaStatus), new object[] { args.ToString() });
        }
        ...
        public override bool sendFuncionario(AssepontoRep.Types.Funcionario Funcionario)
        {
            ...
            person.RecordKeyboard = 1; //(byte)(TerminalDados.Teclado ? 1 : 0);
        ...
        public override bool deleteFuncionario(AssepontoRep.Types.Funcionario Funcionario)
        {
            ...
            LoadList loadList = new LoadList();
            loadList.DeviceID = 5433; //devices.getDeviceId(TerminalDados.IP);
            loadList.CommStatus = 2; 
            loadList.SeqCmd = 10;
            loadList.Type = 9; 
            loadList.List = employeeListsClock;
        ...
        {
            UpdateSummerTime udt = new UpdateSummerTime();
            udt.DeviceID = devices.getDeviceId(TerminalDados.IP);
            udt.SeqCmd = 128;
            udt.CommStatus = 2;
            udt.InitSummerTime = inicio;
            udt.EndSummerTime = fim;
        ...
        public void MenssageClockReceive(MessageReceivedArgsClock args)
        {
            ...
            {
                consoleLog.BeginInvoke(new updateResp(AtualizaResp), new object[] {
                    formatMensagem(devices.getDeviceIP(((UpdateClockResp)args.Msg).DeviceID), "DATA E HORA ATUALIZADO\t", ((ClockResultType)((UpdateClockResp)args.Msg).Status).ToString())                 
                });
